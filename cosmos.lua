...
if ChatExtrasConfig.enabled then
	local function damageLogger(event)
		...
		if (event:GetName() == 'player_hurt' ) then
			...
			local timestamp
			if ChatExtrasConfig.timestamps then
				timestamp = argbColor( ChatExtrasConfig.timestampcolor ) .. "[" .. os.date( '%H:%M' ) .. "] : "
			else
				timestamp = ''
			end
...
if CheatsBypassConfig.enabled then
	local function CheatsBypass()
		client.SetConVar( "sv_cheats", 1 )
		if CheatsBypassConfig.AlwaysDeathAnim then
			client.Command( "tf_always_deathanim 1", true )
		end
	end
	callbacks.Register( "Draw", "CheatsBypass", CheatsBypass )
end
...
if AutoStrafeConfig.enabled then
	local function AutoStrafeCheck()
